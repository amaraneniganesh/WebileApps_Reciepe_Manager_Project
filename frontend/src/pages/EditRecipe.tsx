...
const EditRecipe: React.FC = () => {
    ...
    useEffect(() => {
        const fetchRecipe = async () => {
            try {
                ...
                const data = res.data;
                const ingredientsString = Array.isArray(data.ingredients) ? data.ingredients.join(', ') : data.ingredients;
    ...
    if (fetching) return <div className="text-center mt-20 text-gray-500">Loading recipe details...</div>;
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 sm:p-10">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div>
                        ...
                        <textarea name="ingredients" required rows={3} value={formData.ingredients} onChange={handleChange} className="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 outline-none resize-none"></textarea>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="number" name="cookingTime" required value={formData.cookingTime} onChange={handleChange} className="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 outline-none" />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">Servings</label>
                            <input type="number" name="servings" required value={formData.servings} onChange={handleChange} className="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 outline-none" />
                        </div>
                    </div>

                    <div className="border-2 border-dashed border-gray-200 rounded-xl p-6 bg-gray-50 text-center">
                        <label className="block text-sm font-bold text-gray-700 mb-4">Update Recipe Image</label>
                        {formData.imageUrl && !imageFile && (
                            <img src={formData.imageUrl} alt="Current" className="w-24 h-24 object-cover rounded-lg mx-auto mb-4 border border-gray-200" />
                        )}
                        <label className="cursor-pointer inline-flex items-center gap-2 px-6 py-2 border border-gray-300 rounded-lg text-sm font-medium text-gray-700 bg-white hover:bg-gray-100 transition">
                            <FiUploadCloud size={20} /> Upload New File
                            <input type="file" hidden accept="image/*" onChange={handleFileChange} />
                        </label>
                        <p className="text-gray-400 text-sm my-3 font-medium">OR</p>
                        <input type="text" name="imageUrl" placeholder="Paste New Image URL" value={formData.imageUrl} onChange={handleChange} disabled={!!imageFile} className="w-full max-w-md mx-auto border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 outline-none block text-sm" />
                    </div>
                    ...
                    <div className="flex gap-4 pt-4">
                        ...
                        <button type="button" onClick={() => navigate(`/recipes/${id}`)} className="flex-1 sm:flex-none bg-gray-100 hover:bg-gray-200 text-gray-700 font-medium py-2 px-8 rounded-lg transition">
                            Cancel
                        </button>
                    </div>
                </form>
